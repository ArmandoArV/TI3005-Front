...
export const ReasonModalComponent: React.FC<ReasonModalComponentProps> = ({
  show,
  onClose,
  onSend,
  reason,
  ...
  const handleSend = () => {
    showConfirmAlert(
      ...
      async () => {
        if (onSend) {
          await onSend(ownerId, documentId, ownerType, reasonText);
        }
        onClose();
      }
    );
  ...
  console.log("ReasonModalComponent", {
    show,
    reason,
    ownerId,
    documentId,
